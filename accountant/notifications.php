...
function ensureNotificationsSchema(PDO $pdo): array
{
    $pdo->exec("
    ...
    $requiredColumns = [
        ...
        'title' => "ALTER TABLE notifications ADD COLUMN title VARCHAR(255) NOT NULL AFTER user_id",
        'message' => "ALTER TABLE notifications ADD COLUMN message TEXT NOT NULL AFTER title",
        'type' => "ALTER TABLE notifications ADD COLUMN type VARCHAR(100) NOT NULL DEFAULT 'general' AFTER message",
        'priority' => "ALTER TABLE notifications ADD COLUMN priority VARCHAR(20) NOT NULL DEFAULT 'normal' AFTER type",
        'icon' => "ALTER TABLE notifications ADD COLUMN icon VARCHAR(191) NULL AFTER priority",
        'icon_color' => "ALTER TABLE notifications ADD COLUMN icon_color VARCHAR(50) NULL AFTER icon",
        'related_id' => "ALTER TABLE notifications ADD COLUMN related_id INT NULL AFTER icon_color",
        'related_type' => "ALTER TABLE notifications ADD COLUMN related_type VARCHAR(100) NULL AFTER related_id",
        'is_read' => "ALTER TABLE notifications ADD COLUMN is_read TINYINT(1) NOT NULL DEFAULT 0 AFTER related_type",
        'created_at' => "ALTER TABLE notifications ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP AFTER is_read",
    ...
    return getTableColumns($pdo, 'notifications');
}

function getUsersSchemaConfig(PDO $pdo): array
{
    $columns = getTableColumns($pdo, 'users');

    $fullNameExpression = 'CAST(id AS CHAR)';
    if (isset($columns['full_name'])) {
        $fullNameExpression = 'full_name';
    } elseif (isset($columns['name'])) {
        $fullNameExpression = 'name';
    } elseif (isset($columns['username'])) {
        $fullNameExpression = 'username';
    } elseif (isset($columns['email'])) {
        $fullNameExpression = 'email';
    }

    $roleColumn = null;
    if (isset($columns['role'])) {
        $roleColumn = 'role';
    } elseif (isset($columns['user_role'])) {
    ...
    return [
        'columns' => $columns,
        'full_name_expr' => $fullNameExpression,
        'role_column' => $roleColumn,
    ];
}

$notificationColumns = ensureNotificationsSchema($pdo);
$usersSchema = getUsersSchemaConfig($pdo);
$roleColumn = $usersSchema['role_column'];
$fullNameExpr = $usersSchema['full_name_expr'];
...
$user = $stmt->fetch();
$currentUserRole = $roleColumn && is_array($user) ? ($user[$roleColumn] ?? ($_SESSION['user_role'] ?? '')) : ($_SESSION['user_role'] ?? '');
    ...
    if (isset($_POST['create_notification'])) {
        ...
        if (!empty($title) && !empty($message)) {
            ...
            if ($target_users === 'all') {
                ...
                if (!empty($role) && $roleColumn) {
                    $users_stmt = $pdo->prepare("SELECT id FROM users WHERE {$roleColumn} = ?");
...
if ($currentUserRole === 'admin') {
    $roleSelect = $roleColumn ? "{$roleColumn} AS role" : "'' AS role";
    $users_stmt = $pdo->prepare("SELECT id, {$fullNameExpr} AS full_name, {$roleSelect} FROM users WHERE id != ? ORDER BY full_name");
